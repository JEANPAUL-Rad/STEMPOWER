...
export const create = async (req, res) => {
  try {
    ...
    if (!full_name || !email_address || !level_of_archicad_skills) {
      return res.status(400).json({
        message:
          "full_name, email_address and level_of_archicad_skills are required",
      });
    }
    ...
    if (!allowedLevels.includes(level_of_archicad_skills)) {
      return res.status(400).json({
        message: `level_of_archicad_skills must be one of: ${allowedLevels.join(", ")}`,
      });
    }
      ...
      try {
        ...
        if (registration.payment_status === "Paid" && module) {
          try {
            ...
            console.log(
              `✅ Enrollment created immediately for user ${existingUser.user_id} in module "${module}"`,
            );
          } catch (enrollErr) {
    ...
    if (error?.code === "23514") {
      return res
        .status(400)
        .json({ message: "Invalid data: violates a check constraint" });
    }
    const devInfo =
...
export const updatePaymentStatus = async (req, res) => {
  try {
    ...
    // If payment is confirmed, update last_payment_date in users table and create enrollment
    if (updated.payment_status && updated.payment_status.toLowerCase() === "paid") {
      try {
        // Update last_payment_date in users table for payment tracking
        ...
        if (user_id) {
          // Update last_payment_date and unblock user if blocked
          // Use updated_at from register table if available, otherwise use current timestamp
          const paymentDate = updated.updated_at || new Date();
          await sql`
            UPDATE users 
          ...
          `;
          console.log(`✅ Updated last_payment_date for user ${user_id} to ${paymentDate}`);
        }
      } catch (updateError) {
        console.error('Error updating last_payment_date in users table:', updateError);
        // Don't fail payment update if this fails
      }
    }

    // If payment is confirmed, create enrollment for the user
    if (updated.payment_status && updated.payment_status.toLowerCase() === "paid" && updated.module) {
      try {
        ...
        if (!user_id) {
          ...
          if (user) {
            ...
            console.log(
              `Payment confirmed for registration ${id}, but user account doesn't exist yet. Enrollment will be created when user confirms their account.`,
            );
          ...
          if (existingEnrollmentForReg.length === 0) {
            ...
            if (existingActiveEnrollments.length > 0) {
              ...
              await sql`
							...
							`;
              console.log(
                `⚠️ User ${user_id} had active enrollment in "${existingModule}" - cancelled to enroll in "${updated.module}"`,
              );
            }
            ...
            try {
              ...
              console.log(
                `✅ Enrollment created for user ${user_id} in module "${updated.module}" (Enrollment ID: ${enrollment?.enrollment_id || "N/A"})`,
              );
              console.log(
                `   Registration ID: ${id}, Module: ${updated.module}`,
              );
            } catch (enrollmentErr) {
            ...
            console.log(
              `ℹ️ Enrollment already exists for user ${user_id} in module "${updated.module}"`,
            );
          }
        } else {
          if (!user_id) {
            console.warn(
              `⚠️ Cannot create enrollment: user_id is missing for registration ${id}. Enrollment will be created when user confirms email.`,
            );
          } else if (!updated.module) {
            console.warn(
              `⚠️ Cannot create enrollment: module is missing for registration ${id}`,
            );
          }
        }
...
export const update = async (req, res) => {
  try {
    ...
    const needsEnrollmentSync =
      moduleChanged || (updated.payment_status === "Paid" && updated.module);
    ...
    if (needsEnrollmentSync && updated.module) {
      try {
        ...
        if (user_id && updated.module) {
          ...
          const existingModule = hasExistingEnrollment
            ? existingEnrollment[0].module
            : null;
          const moduleNeedsUpdate =
            hasExistingEnrollment && existingModule !== updated.module;
          ...
          if (moduleNeedsUpdate) {
            await sql`
						...
						`;
            console.log(
              `⚠️ Cancelled enrollment in old module "${existingModule}" for user ${user_id}`,
            );
          }
          ...
          if (updated.payment_status && updated.payment_status.toLowerCase() === "paid") {
            ...
            if (existingEnrollmentForNewModule.length === 0) {
              ...
              console.log(
                `✅ Enrollment created/updated for user ${user_id} in module "${updated.module}" (Enrollment ID: ${enrollment?.enrollment_id || "N/A"})`,
              );
              console.log(
                `   Registration ID: ${id}, Module: ${updated.module}`,
              );
              ...
              if (
                existingEnrollmentForNewModule[0].enrollment_id &&
                (!hasExistingEnrollment ||
                  existingEnrollment[0].registration_id !== parseInt(id))
              ) {
                await sql`
									UPDATE enrollments
								...
								`;
                console.log(
                  `✅ Updated enrollment registration_id for user ${user_id} in module "${updated.module}"`,
                );
              } else {
                console.log(
                  `ℹ️ Enrollment already exists for user ${user_id} in module "${updated.module}"`,
            ...
            console.log(
              `ℹ️ Module changed but payment not Paid - enrollment will be created when payment is confirmed`,
            );
          }
        } else {
          if (!user_id) {
            console.warn(
              `⚠️ Cannot sync enrollment: user_id is missing for registration ${id}. Enrollment will be created when user confirms email.`,
            );
          } else if (!updated.module) {
            console.warn(
              `⚠️ Cannot sync enrollment: module is missing for registration ${id}`,
            );
          }
        }
      } catch (enrollmentError) {
        ...
        console.error(
          "❌ Error syncing enrollment after module update:",
          enrollmentError,
        );
        console.error("Error details:", {
          message: enrollmentError.message,
