...
if (isPooler) {
  // POOLER CONNECTION - Force Session mode ONLY (port 5432)
  const portMatch = connectionString.match(/:(\d+)\//);
  const port = portMatch ? portMatch[1] : null;
  
  if (port === '6543') {
    // Port 6543 = Transaction mode - FORCE conversion to Session mode (port 5432)
    console.log('🔄 Detected Pooler Transaction mode (port 6543) - FORCING conversion to Session mode (port 5432)');
    console.log('   Transaction mode is not supported - using Session mode only');
    
    // Replace port 6543 with 5432
    finalConnectionString = connectionString.replace(':6543/', ':5432/');
    
    // Remove pgbouncer parameter if present (not needed for session mode)
    finalConnectionString = finalConnectionString.replace(/[?&]pgbouncer=true/, '');
    
    console.log('✅ Successfully switched to Session mode (port 5432)');
  } else if (port === '5432') {
    // Port 5432 = Session mode (correct mode)
    console.log('✅ Using Supabase pooler in Session mode (port 5432)');
    
    // Remove pgbouncer parameter if present (not compatible with session mode)
    if (finalConnectionString.includes('pgbouncer=true')) {
      finalConnectionString = finalConnectionString.replace(/[?&]pgbouncer=true/, '');
      console.log('   Removed pgbouncer=true parameter (not needed for Session mode)');
    }
  } else {
    // No port specified - default to Session mode (port 5432)
    console.log(`⚠️  No port specified in pooler URL - defaulting to Session mode (port 5432)`);
    
    // Add port 5432 if no port specified
    if (!finalConnectionString.match(/:\d+\//)) {
      finalConnectionString = finalConnectionString.replace(/\.pooler\.supabase\.com\//, '.pooler.supabase.com:5432/');
    }
  ...
  // Set max connections for Session mode (very limited: 1-2 connections max)
  maxConnections = parseInt(process.env.DB_MAX_CONNECTIONS || '1', 10);
  console.log(`📊 Session mode connection pool: max=${maxConnections} connections (limited by Supabase)`);
  console.log('⚠️  Session mode has strict connection limits - keep max connections LOW');
  ...
  // Direct connections: use very conservative connection pool
...
const sql = postgres(finalConnectionString, {
  ...
  max_lifetime: 60 * 30, // Close connections after 30 minutes (prevents stale connections)
  prepare: true, // Enable prepared statements for Session mode (better performance)
  transform: {
    undefined: null, // Transform undefined to null for PostgreSQL compatibility
  },
  connection: {
...
export const checkConnectionHealth = async () => {
  try {
    ...
    if (error.message?.includes('CONNECT_TIMEOUT')) {
      ...
      console.error('\n   Try:');
      console.error('   1. Verify your DATABASE_URL in .env file');
      console.error('   2. Ensure you are using Session mode (port 5432)');
      console.error('   3. Check if you can reach the Supabase dashboard');
      console.error('   4. Increase DB_CONNECT_TIMEOUT environment variable');
    }
    return false;
  }
...
export const handleConnectionError = (error) => {
  ...
  if (error.message?.includes('max clients reached') || error.message?.includes('MaxClientsInSessionMode')) {
    return {
      error: 'Database connection pool exhausted',
      message: 'Too many concurrent connections in Session mode. Reduce DB_MAX_CONNECTIONS or wait for connections to close.',
      retryable: true,
      code: 'POOL_EXHAUSTED'
    };
  }
